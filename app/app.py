...
def load_artifacts():
    ...
    return model, scaler, model_columns, numeric_cols, brand_list, vehicle_type_list

try:
    model, scaler, model_columns, numeric_cols, brand_list, vehicle_type_list = load_artifacts()
except FileNotFoundError:
    st.error(
        "Model files not found. Please place lasso_model.pkl, scaler.pkl, "
        "model_columns.pkl, numeric_cols.pkl, brand_list.pkl, and "
        "vehicle_type_list.pkl in the same folder as this app."
    )
    st.stop()
...
# ---------------------------------------------------------------
def predict_price(brand, kilometer, vehicle_age, transmission_type, unrepaired_damage, vehicle_type, power_ps):
...
if st.button("Estimate Price", type="primary", use_container_width=True):
    ...
    # Updated to the improved model's test RMSE (power_ps + vehicle_type added)
    margin = 5492.83
